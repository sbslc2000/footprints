...
```
단순 스프링만 이용하면 JpaRepository 빈을 등록하기 위해서는 @EnableJpaRepositories 애노테이션을 Configuration 클래스에 작성해줘야 한다.

## JpaRepositoriesRegistrar
JpaRepositoriesRegistrar 클래스는 스프링이 지원하는 ImportBeanDefinitionRegistrar을 구현한 클래스이다. ImportBeanDefinitionRegistrar은 런타임에 객체를 만들고 빈을 등록해주는 기능을 한다.
