...
### 명령어의 가공
사용자가 작성하는 명령어, 예를 들어 python main.py를 그대로 컨테이너에게 전달하는 경우 도커 컨테이너에서는 원하는 응답을 얻어내기 힘듦니다. main.py라는 파일은 도커 컨테이너의 볼륨 디렉터리에 들어있을 것이므로, 사용자의 요청은 python /{volumePath}/main.py로 바뀌어야 할 것입니다.

또한 cd를 통해 디렉터리의 현재 위치를 바꾼 경우에도 python main.py는 원래의 의도대로 동작하지 않을 가능성이 있습니다. 따라서 클라이언트로부터 받는 명령어를 가공해야할 필요가 있었습니다.

명령어의 가공은 요구사항으로부터 도출됩니다. 터미널 기능 구현시 요구사항은 다음과 같습니다.
