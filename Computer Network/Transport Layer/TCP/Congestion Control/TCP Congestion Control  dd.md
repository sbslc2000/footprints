* 실시간 멀티미디어 서비스에서 TCP를 사용하면, 네트워크 혼잡할 때, 전송량을 알아서 낮춤.


1. UDP -> 전송속도를 application에서 제어
	1. ex. 송신자(앱)가 1초에 2MB -> (문제가 있지 않은 한) 수신자도 1초에 2MB
2. TCP -> 전송속도를 Transport Layer에서 제어
	1. ex. 송신자(앱)가 1초에 2MB -> 수신자 1초에 2MB
	2. 왜?
		1. **라우터가 혼잡하면 - 속도 낮춤**
		2. **라우터가 혼잡하지 않으면 - 속도 올림**
